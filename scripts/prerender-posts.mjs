...
async function main() {
    ...
    // Manifest consumed by the blog listing at runtime. Embeds title / date /
    // summary per locale so the listing doesn't have to fetch each markdown
    // body. Sorted newest-first by slug.
    const indexPosts = [];

    let count = 0;
    for (const slug of slugs) {
        const entry = { slug, locales: {} };
        ...
        for (const lang of['en', 'pt']) {
            ...
            entry.locales[lang] = { title, date: fm.date || '', summary };
        ...
        if (Object.keys(entry.locales).length) indexPosts.push(entry);
    }

    indexPosts.sort((a, b) => b.slug.localeCompare(a.slug));
    await writeFile(
        join(OUT_DIR, '_posts.json'),
        JSON.stringify({ posts: indexPosts }, null, 2),
        'utf8'
    );

    console.log(`Pre-rendered ${count} post page(s) and manifest to /p/`);
}
